...
export default function NewItemPage() {
  ...
  const preselectCategory = search?.get('category') || '';
  ...
  const [categoryId, setCategoryId] = useState(preselectCategory);
  ...
  // Images are managed by ImageUploader and already uploaded to Cloudinary
  ...
  if (loading) {
    return <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex items-center justify-center dark:text-slate-400">Loading...</div>;
  }

  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950">
      <div className="container mx-auto px-4 py-8 max-w-3xl">
        <h1 className="mb-6 text-2xl font-bold text-slate-900 dark:text-slate-100">New Item</h1>
        <form onSubmit={handleSubmit} className="space-y-4 rounded-lg border bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          ...
          <div>
            <label className="mb-1 block text-sm font-medium text-slate-700 dark:text-slate-300">Images</label>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <label className="flex items-center gap-2">
              ...
              <span className="text-sm text-slate-700 dark:text-slate-300">Orderable (visible)</span>
            </label>
            <label className="flex items-center gap-2">
          ...
          <div>
            <div className="mb-2 flex items-center justify-between">
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">Modifiers</label>
            ...
            <div className="space-y-2">
              {modifiers.map((m) => (
                <div key={m.id} className="grid gap-2 md:grid-cols-5 items-center">
                  <input
                    ...
                    className="md:col-span-3 rounded-lg border border-slate-300 px-3 py-2 focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100"
              ...
              {modifiers.length === 0 && <p className="text-xs text-slate-500 dark:text-slate-400">No modifiers added.</p>}
            </div>
          </div>
