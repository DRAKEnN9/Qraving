      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-slate-100 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-800">
        <div className="flex items-center gap-3">
          <button
            {...attributes}
            {...listeners}
            className="cursor-grab text-slate-400 hover:text-slate-600 active:cursor-grabbing dark:text-slate-400 dark:hover:text-slate-200"
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="rounded-lg p-2 text-slate-400 hover:bg-red-100 hover:text-red-600 dark:text-slate-400 dark:hover:bg-red-900/20"
      ...
      <div className="p-4">
        {!category.items || category.items.length === 0 ? (
          <div className="py-8 text-center">
            ...
            <p className="mb-4 text-slate-500 dark:text-slate-400">No items in this category yet</p>
            <Link
              href={`/dashboard/menu-builder/${restaurant._id}/items/new?category=${category._id}`}
          ...
          <div className="space-y-3">
                  ...
                  <div className="h-16 w-16 flex-shrink-0">
                    {item.images?.[0] ? (
                      ...
                      <div className="flex h-16 w-16 items-center justify-center rounded-lg bg-slate-100 dark:bg-slate-800">
                        <ImageIcon className="h-6 w-6 text-slate-400 dark:text-slate-400" />
                      </div>
                    )}
                  </div>
            ...
            <Link
              href={`/dashboard/menu-builder/${restaurant._id}/items/new?category=${category._id}`}
              className="flex w-full items-center justify-center gap-2 rounded-lg border-2 border-dashed border-slate-300 py-4 text-slate-600 transition-colors hover:border-emerald-300 hover:text-emerald-600 dark:border-slate-700 dark:text-slate-300 dark:hover:border-emerald-400 dark:hover:text-emerald-400"
            >
              <Plus className="h-4 w-4" />
...
export default function MenuBuilderPage() {
  ...
  const fetchMenuData = async () => {
    try {
      ...
      const categoriesWithItems = (categoriesData.categories || []).map((cat: Category) => ({
        ...cat,
        items: (itemsData.menuItems || []).filter(
          (item: MenuItem) => String(item.categoryId) === String(cat._id)
        ),
      }));
  ...
  const handleToggleSoldOut = async (item: MenuItem, categoryId: string) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`/api/owner/menu-items/${item._id}`, {
        method: 'PATCH',
        headers: {
  ...
  const handleEditCategory = (category: Category) => {
    router.push(`/dashboard/menu-builder/${restaurantId}/categories/${category._id}/edit`);
  };

  const handleDeleteCategory = async (categoryId: string) => {
    if (
      !confirm(
        'Are you sure you want to delete this category? All items in it will be deleted as well.'
      )
    ) {
      return;
    }
  ...
  const handleEditItem = (item: MenuItem, categoryId: string) => {
    router.push(
      `/dashboard/menu-builder/${restaurantId}/items/${item._id}/edit?category=${categoryId}`
    );
  };
      ...
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950">
        <div className="container mx-auto px-4 py-8">
          ...
          <div className="space-y-6">
            {[1, 2, 3].map((i) => (
              <div key={i} className="h-64 animate-pulse rounded-lg bg-slate-200"></div>
            ))}
          </div>
        </div>
      </div>
      ...
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950">
        <div className="container mx-auto px-4 py-8">
          <div className="text-center">
            ...
            <h1 className="mb-2 text-2xl font-bold text-slate-900">Error Loading Menu</h1>
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:gap-3 w-full sm:w-auto">
              <Link
                ...
                target="_blank"
                className="flex items-center gap-2 rounded-lg border border-slate-300 px-4 py-2 font-medium text-slate-700 transition-colors hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200 dark:hover:bg-slate-700"
              >
                <Eye className="h-4 w-4" />
                Preview Menu
              </Link>
              <Link
                href={`/dashboard/menu-builder/${restaurantId}/categories/new`}
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-slate-900 dark:text-slate-100">
                    {categories.reduce(
                      (sum, cat) =>
                        sum +
                        (cat.items?.filter((item) => item.orderable && !item.soldOut).length || 0),
                      0
                    )}
                  </p>
                </div>
